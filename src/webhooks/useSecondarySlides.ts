 ...
 * @returns {useSecondarySlides} - The object returned from the webhook.
 */
const useSecondarySlides = ({location}:Props): useSecondarySlides => {
    const [secondarySlides, setSecondarySlides] = useState<Slides[]>([]);
    const [secondarySlideError, setError] = useState('');

    useEffect(() => {
      const controller = new AbortController();
      axios.get(`/api/${location}`, { signal: controller.signal })
        .then(response => {
